...
Server: `https://mcp.heypastel.com/mcp` (OAuth; the host handles auth). Use the live tool names the server advertises; this file maps them to jobs. `pastel_list_capabilities` is the live contract — when this file and that response disagree, the response wins.
...
- **Missing scope.** When capabilities shows an action with `available: false`, name its `missing_scopes` and tell the user to reconnect the Pastel MCP granting them. Continue with read-and-draft work meanwhile. A read tool can fail the same way (`scope_missing`, e.g. `sequences:read`): skip that section and list it under **Unknowns**.
...
3. An authentication error ends all Pastel requests for the run; ask the user to reconnect the Pastel MCP.
